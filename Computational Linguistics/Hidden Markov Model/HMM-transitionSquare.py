...
def main(argv):
	...
	for x in range(1,10):
		for y in range(1,10):
			transitions = {0 : float(x)/10.0, 1 : float(10-x)/10.0}
			transitions2 = {0 : float(y)/10.0, 1 : float(10-y)/10.0}
			state0 = state(createRandomDistribution(alphabet), transitions, 0)
			state1 = state(createRandomDistribution(alphabet), transitions2, 1)
			...
			############################################
			print("X : %d Y: %d" % (x,y))
			plogList.append([(x/10.0 , (10-x)/10.0), (y/10.0 , (10-y)/10.0), totalPlog])
			#printInitialization(states[0],states[1],Pi, False)
			#sortedLogRation = logRatio(state0,state1)
			#displayLogRatio(sortedLogRation)
	
	displayPlogList(plogList)
	print("finished")

def displayPlogList(plogList):
	plogList.sort(key = lambda val : val[2])
	for item in plogList:
		print(item)
