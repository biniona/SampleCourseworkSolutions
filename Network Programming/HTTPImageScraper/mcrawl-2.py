...
from urlSocketFunctions import parseURLToHostNameAndFilePath, createSocket, isHTML

def main(argv):
    #main code 

    #declare global variables  
    global qLock, fileNameLock, THREAD_NUM, server, port, dirName, crawled, q, localDirectoryName
    
    #this is to test if necessary globals are actually assigned in opt args
    optFlagsArr = [False,False,False,False]
    
    #get opt args and assign globals
    try:
       opts, args = getopt.getopt(argv,"n:h:p:f:")
    except getopt.GetoptError:
       print('mcrawl-2.py -n [ -n max-flows ] [ -h hostname ] [ -p port ] [-f local- directory]')
       sys.exit(2)
    for opt, arg in opts:
        if opt == '-n':
           optFlagsArr[0] = True
    ...
    if s is None:
        print("Bad Hostname")
        sys.exit(2)
    firstCookie = crawl_links(server, "drawings", s, dirName, q, crawled, "NONE", 0)
    s.close()
    threads = []
    workingThreads = []
...
def parse(workingThreads, threadNum):
    global q, server, dirName, crawled, port
    cookie = "NONE"
    while True:
        while not q.empty():
            ...
            if(parsed[0] == 1):
                print(".jpg in {} = {}".format(parsed[2], (".jpg" in parsed[2])))
                if ((parsed[1] == server) or (".jpg" in parsed[2])):
                    newSocket = createSocket(server, port)
                    if (newSocket != None):
                        cookie = crawl_links(server, parsed[2], newSocket, dirName, q, crawled, cookie, threadNum)
                        newSocket.close()
